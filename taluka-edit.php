...
// Get and decode the taluka_id from URL parameter
$encoded_id = isset($_GET['id']) ? $_GET['id'] : '';
$taluka_id = '';

if ($encoded_id) {
    $taluka_id = base64_decode($encoded_id);
    if (!$taluka_id || !is_numeric($taluka_id)) {
        echo "<script>showToast('Invalid taluka ID', false); setTimeout(() => { window.location.href = 'taluka-list'; }, 2000);</script>";
        exit();
    }
} else {
    echo "<script>showToast('No taluka ID provided', false); setTimeout(() => { window.location.href = 'taluka-list'; }, 2000);</script>";
    exit();
}
?>
  ...
  <div class="card-body">
    ...
    <form method="post" id="talukaForm">
      ...
      <div class="form-group mb-3">
        ...
        <label for="taluka_name" class="form-label">Taluka</label>
...
$(document).ready(function() {
  ...
  // Use the taluka_id from PHP which was decoded from the URL
  var talukaId = '<?php echo $taluka_id; ?>';
